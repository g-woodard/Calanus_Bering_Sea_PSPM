...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    netproduction = sigma*Ingest/.322 - (Metabolic_rate*(.014196/0.0148218)) #need to convert ingestion from carbon to grams of prey, and metabolism from oxygen consumed to grams of prey
 
    #0.455 is conversion factor from Uye 1982 where COPEPOD dry weight is 45.5% Carbon
    #IT MAY BE WE NEED TO USE PHTYOPLANKTON CARBON TO DRY WEIGHT RATIO HERE AS WELL?????? THEN IT IS THE ASSIMILATION EFFICIENCY THAT ADDRESSES CONVERSION TO BIOMASS
    #SO I'M  THINKING THE .455 VALUE AND THE .0279 VALUE SHOULD BE FOR PHYTOPLANKTON, SO CARBON:DW AND PHYTOPLANKTON ENERGY DENSITY
    
    #Average proportion carbon for phytoplankton dry weight is .322 (Platt and Irwin 1972)
    #Average Joules per microgram phytoplankton dry weight is 0.0148218 (Platt and Irwin 1972; converted from calories)
    
    #0.014196 is oxycalorific coefficient where 1 ug of O2 = .014196 J (Elliot and Davison 1975).
    ...
    #WAAAIT SHOULD THE COPEPOD ENERGY DENSITY ACTUALLY BE THE PREY (PHYTOPLANKTON ENERGY DENSITY????)
