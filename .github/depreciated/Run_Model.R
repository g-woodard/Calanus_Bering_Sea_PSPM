...
I_only_per_capita  <- per_capita_I_DF %>%
  filter(Measurement == "per_capita_I") %>%
  mutate(Measurement = str_replace(Measurement, "per_capita_I","I")) %>%
  rename("value" = "Value") %>%
  mutate(Rate = "Per Capita")


Imax_only <- I_DF %>%
  filter(Measurement == "Imax") %>%
  mutate(Rate = "Population")


Imax_only_per_capita <- per_capita_I_DF %>%
  filter(Measurement == "per_capita_Imax") %>%
  mutate(Measurement = str_replace(Measurement, "per_capita_Imax","Imax")) %>%
  rename("value" = "Value") %>%
  mutate(Rate = "Per Capita")
...
parameter_rates_graph <- ggarrange(per_capita_turnover_graph, per_capita_ingestion_graph, per_capita_mortality_graph, per_capita_metabolism_graph,
                                   total_turnover_graph, total_ingestion_graph, total_mortality_graph, total_metabolism_graph,  ncol = 4, nrow = 2, align = "v", labels = c("A", "B", "C", "D", "E", "F","G","H"),
...
test_graph <- ggarrange(parameter_rates_graph_alt, turnovergraphs, nrow = 2)
test_graph 
...
parameter_rates_graph <- annotate_figure(parameter_rates_graph,
                ...
                #left = text_grob("µg / day", size = 13, rot = 90)
                )
...
mj[which(Total_biomass == max(Total_biomass, na.rm = TRUE))]
